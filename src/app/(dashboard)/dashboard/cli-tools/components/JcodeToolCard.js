...
export default function JcodeToolCard({
  ...
  const getConfigStatus = () => {
    ...
    if (!jcodeStatus?.hasExtremeRouter) return "not_configured";
    const currentProvider = jcodeStatus.config?.providers?.["@rsalmn/extremerouter"];
  ...
  useEffect(() => {
    if (jcodeStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = jcodeStatus.config?.providers?.["@rsalmn/extremerouter"];
      if (provider) {
  ...
  const getManualConfigs = () => {
    ...
    const configToml = `[providers.extremerouter]
...
id = "${selectedModel || "cc/claude-opus-4-7"}"`;
    ...
    const envContent = `JCODE_EXTREMEROUTER_API_KEY="${keyToUse}"`;
    ...
    return [
      ...
      {
        filename: "~/.config/jcode/provider-extremerouter.env",
        content: envContent,
      },
    ];
  };
          ...
          {!checkingJcode && jcodeStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {jcodeStatus?.config?.providers?.["@rsalmn/extremerouter"]?.base_url && (
                ...
                <div className="flex flex-col gap-1 p-3 bg-blue-500/5 border border-blue-500/20 rounded-lg">
                  ...
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile extremerouter</code>
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile extremerouter --model {selectedModel || "cc/claude-opus-4-7"}</code>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!jcodeStatus?.hasExtremeRouter} loading={restoring}>
