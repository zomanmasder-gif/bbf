...
function buildMenuItems(port, autostartEnabled) {
  return [
    { title: `ExtremeRouter (Port ${port})`, tooltip: "Server is running", enabled: false },
...
function initWindowsTray(options) {
  ...
  try {
    ...
    trayInstance = initWinTray({
      iconPath,
      tooltip: `ExtremeRouter - Port ${port}`,
      items,
      onClick: (index) => {
  ...
  try {
    ...
    if (isV2) {
      ...
      trayInstance.ready().catch((err) => {
        process.stderr.write(`[extremerouter] tray failed to start: ${err && err.message ? err.message : err}\n`);
      });
    } else {
    ...
    process.stderr.write(`[extremerouter] tray init error: ${err.message}\n`);
    return null;
  }
}
